...
int main(/*int argc,char* argv[]*/)
{
    ...
    /*while (true) {*/
        ...
        //ifstream ifsp;
        //ifsp.open("./time_position/.txt", ios::in);
        //char bufp[1024];
        //while (ifsc.getline(bufc, sizeof(bufp)))
        //{
        //    if (bufc != "\n")
        //    {
        //        vector<double> s;
        //        char* next_token;
        //        char* token = strtok_s(bufp, " ", &next_token);
        //        double num;
        //        while (token != NULL) {
        //            const char* ptr = token;
        //            char st[100] = { 0 };
        ...
        //cout << GNLs[1].second.size() << endl;

        /*for (auto it = GNLs[1].first.middle_point.begin(); it != GNLs[1].first.middle_point.end(); it++) {
